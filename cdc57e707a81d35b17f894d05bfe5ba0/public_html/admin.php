...
if (isset($_POST['add_kitten'])) {
    ...
    if (isset($_FILES['imageUpload']) && $_FILES['imageUpload']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_ext, $allowed_ext)) {
            $new_file_name = uniqid('kitten_', true) . '.' . $file_ext;
            $file_path = $upload_dir . $new_file_name;
            
            if (move_uploaded_file($file_tmp, $file_path)) {
                $image_url = $file_path;
            } else {
                $message = "<div class='alert alert--error'>Ошибка загрузки файла.</div>";
            }
        } else {
            $message = "<div class='alert alert--error'>Недопустимый формат файла.</div>";
        }
    }
    ...
    // Подготовка запроса (остается без изменений)
...
<nav class="navbar">
    <ul class="navbar-list">
        ...
        <li class="navbar-item"><a href="change_credentials.php" class="navbar-link">Изменить логин/пароль</a></li>
...
<h2 id="contacts">Данные с формы "Вопросы"</h2>
<table>
    <tr>
        <th>ID</th>
        <th>Имя</th>
